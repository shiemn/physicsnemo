...
def stochastic_sampler(
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
        ...
        # Handle heteroscedastic models that return (mean, std) tuple
...
def uncertainty_aware_stochastic_sampler(
    net: torch.nn.Module,
    latents: Tensor,
    img_lr: Tensor,
    class_labels: Optional[Tensor] = None,
    ...
    S_max: float = float("inf"),
    S_noise: float = 1,
    use_predicted_uncertainty: bool = True,
    uncertainty_scale: float = 1.0,
) -> Tensor:
    r"""
    Uncertainty-aware EDM sampler that uses predicted variance to modulate noise injection.

    This is an extension of the standard EDM stochastic sampler (Algorithm 2) that
    can use the network's predicted uncertainty to spatially modulate the churn noise.
    This allows for calibrated ensemble spread when the network is trained with a
    Gaussian CRPS loss.

    When `use_predicted_uncertainty=True`, the sampler expects the network to return
    a tuple `(denoised_mean, predicted_std)`. The predicted std is used to scale
    the noise injection at each step, allowing regions with higher uncertainty to
    have more variability in the generated samples.

    Parameters
    ----------
    net : torch.nn.Module
        Neural network model. If `use_predicted_uncertainty=True`, the model should
        return `(denoised_mean, predicted_std)` tuple. Otherwise, returns just the
        denoised prediction.
    latents : Tensor
        Latent variables (noise) of shape :math:`(B, C_{out}, H, W)`.
    img_lr : Tensor
        Low-resolution conditioning image of shape :math:`(B, C_{lr}, H, W)`.
    class_labels : Optional[Tensor], optional
        Class labels for conditional generation, by default None.
    randn_like : Callable[[Tensor], Tensor]
        Function to generate random noise, by default ``torch.randn_like``.
    patching : Optional[GridPatching2D], optional
        Patching utility for patch-based diffusion, by default None.
    mean_hr : Optional[Tensor], optional
        Mean high-resolution image for conditioning, by default None.
    lead_time_label : Optional[Tensor], optional
        Lead time labels, by default None.
    num_steps : int
    ...
    sigma_max : float
        Maximum noise level, by default 800.
    rho : float
        Exponent for time step discretization, by default 7.
    S_churn : float
        Base churn parameter, by default 0.
    S_min : float
        Minimum time for applying churn, by default 0.
    S_max : float
        Maximum time for applying churn, by default inf.
    S_noise : float
        Base noise scaling factor, by default 1.
    use_predicted_uncertainty : bool
        Whether to use the network's predicted uncertainty to modulate noise.
        If True, expects network to return (mean, std) tuple.
        If False, behaves like standard stochastic_sampler.
        By default True.
    uncertainty_scale : float
        Scaling factor for the predicted uncertainty, by default 1.0.
        Higher values increase the influence of predicted uncertainty.

    Returns
    -------
    Tensor
        Generated samples of shape :math:`(B, C_{out}, H, W)`.

    Note
    ----
    When `use_predicted_uncertainty=True`, the noise injection at each step is:

    .. math::
        x_{hat} = x_{cur} + \\sqrt{t_{hat}^2 - t_{cur}^2} \\cdot S_{noise} \\cdot
                  (\\alpha \\cdot D_{std} + (1-\\alpha)) \\cdot \\epsilon

    where :math:`\\alpha` is the uncertainty_scale and :math:`D_{std}` is the
    network's predicted standard deviation. This allows regions with higher
    predicted uncertainty to have more stochasticity.
    ...
    - Gneiting & Raftery, 2007. "Strictly proper scoring rules, prediction,
      and estimation"
    """
    # Adjust noise levels based on network support
    sigma_min = max(sigma_min, net.sigma_min)
    sigma_max = min(sigma_max, net.sigma_max)
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        ...
        # Standard churn computation
        gamma = S_churn / num_steps if S_min <= t_cur <= S_max else 0
        t_hat = net.round_sigma(t_cur + gamma * t_cur)

        # Base noise scale
        noise_scale = (t_hat**2 - t_cur**2).sqrt() * S_noise

        # Generate noise
        noise = randn_like(x_cur)

        # If using predicted uncertainty and this is not the final step
        if use_predicted_uncertainty and gamma > 0:
            # Get network prediction with uncertainty
            x_hat_batch = _apply_wrapper_select(input=x_cur, patching=patching)(
                patching=patching, input=x_cur
            ).to(latents.device)

            x_lr = x_lr.to(latents.device)

            # Call network expecting (mean, std) tuple
            net_output = net(
                x_hat_batch,
                x_lr,
                t_cur,  # Use current sigma for uncertainty prediction
                class_labels,
                embedding_selector=patch_embedding_selector,
            ...
            if isinstance(net_output, tuple):
                ...
                if patching:
                    D_std = _fuse_wrapper(
                        patching=patching, input=D_std, batch_size=batch_size
                    )
                # Modulate noise with predicted uncertainty
                # Blend between uniform and predicted uncertainty
                uncertainty_modulation = (
                    uncertainty_scale * D_std + (1 - uncertainty_scale)
                )
                x_hat = x_cur + noise_scale * uncertainty_modulation * noise
            else:
                # Network doesn't support uncertainty, fall back to standard
                x_hat = x_cur + noise_scale * noise
        else:
            # Standard noise injection
            x_hat = x_cur + noise_scale * noise

        # Euler step - get denoised prediction
        x_hat_batch = _apply_wrapper_select(input=x_hat, patching=patching)(
            patching=patching, input=x_hat
        ).to(latents.device)
        ...
        net_output = net(
            ...
            class_labels,
            embedding_selector=patch_embedding_selector,
            return_variance=False,  # Only need mean for denoising step
            **optional_args,
        )

        # Handle tuple output (heteroscedastic network)
        if isinstance(net_output, tuple):
            denoised = net_output[0]
        else:
            denoised = net_output
...
def calibrated_stochastic_sampler(
    net: torch.nn.Module,
    latents: Tensor,
    img_lr: Tensor,
    class_labels: Optional[Tensor] = None,
    ...
    r"""
    Calibrated EDM sampler with post-hoc spread control via temperature scaling
    and D_std-based spatial modulation.

    This sampler decouples three concerns:
    - **Spatial pattern**: D_std from the variance head tells the sampler *where*
      to inject more or less noise (normalized to mean=1, so magnitude doesn't matter).
    - **Overall spread**: Temperature T scales all noise uniformly. T=1 gives
      standard EDM behavior; higher T = more ensemble diversity.
    - **Tail shape**: The diffusion process itself produces non-Gaussian samples
      with realistic spatial correlations — no Gaussian assumption is imposed.

    The noise injection at each churn step is:

    .. math::
        x_{hat} = x_{cur} + \sqrt{t_{hat}^2 - t_{cur}^2} \cdot S_{noise}
                  \cdot T \cdot [\alpha \cdot \hat{D}_{std} + (1-\alpha)]
                  \cdot \epsilon

    where :math:`\hat{D}_{std} = D_{std} / \text{mean}(D_{std})` is the
    mean-normalized spatial uncertainty pattern, T is the temperature, and
    :math:`\alpha` controls spatial modulation strength.

    Parameters
    ----------
    net : torch.nn.Module
        Neural network. If it supports ``return_variance=True`` and returns
        ``(D_mean, D_std)``, the spatial pattern from D_std is used.
        Otherwise, falls back to spatially uniform noise.
    latents : Tensor
        Initial noise of shape ``(B, C_out, H, W)``.
    img_lr : Tensor
        Low-resolution conditioning of shape ``(B, C_lr, H, W)``.
    class_labels : Optional[Tensor]
        Class labels for conditional generation. Default None.
    randn_like : Callable
        Noise generator. Default ``torch.randn_like``.
    patching : Optional[GridPatching2D]
        Patching utility for patch-based diffusion. Default None.
    mean_hr : Optional[Tensor]
        High-resolution mean for conditioning. Default None.
    lead_time_label : Optional[Tensor]
        Lead time labels. Default None.
    num_steps : int
        Number of sampling steps. Default 18.
    sigma_min : float
        Minimum noise level. Default 0.002.
    sigma_max : float
        Maximum noise level. Default 800.
    rho : float
        Time step discretization exponent. Default 7.
    S_churn : float
        Base churn parameter. Default 0.
    S_min : float
        Minimum time for churn. Default 0.
    S_max : float
        Maximum time for churn. Default inf.
    S_noise : float
        Base noise scaling. Default 1.
    temperature : float
        Overall spread scaling. T=1 gives standard EDM spread; T>1 increases
        ensemble diversity; T<1 reduces it. This is the main knob to control
        ensemble calibration and can be tuned post-training. Default 1.0.
    spatial_alpha : float
        Spatial modulation strength. alpha=0 gives uniform noise (temperature
        only); alpha=1 fully modulates by normalized D_std pattern.
        Default 1.0.

    Returns
    -------
    Tensor
        Generated sample of shape ``(B, C_out, H, W)``.

    Note
    ----
    Temperature T and spatial_alpha are inference-time parameters that do not
    require retraining. You can generate ensembles with different T values
    from the same checkpoint to explore the spread-skill tradeoff, then pick
    the T that minimizes CRPS on a validation set.
    """
    # Adjust noise levels
    sigma_min = max(sigma_min, net.sigma_min)
    sigma_max = min(sigma_max, net.sigma_max)
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):
        ...
        # Base noise scale, multiplied by temperature
        noise_scale = (t_hat**2 - t_cur**2).sqrt() * S_noise * temperature
        noise = randn_like(x_cur)

        # Get D_std for spatial modulation (only if churn is active)
        if spatial_alpha > 0 and gamma > 0:
            x_cur_batch = _apply_wrapper_select(input=x_cur, patching=patching)(
                patching=patching, input=x_cur
            ).to(latents.device)
            x_lr = x_lr.to(latents.device)

            net_output = net(
                x_cur_batch, x_lr, t_cur, class_labels,
                embedding_selector=patch_embedding_selector,
                return_variance=True,
                **optional_args,
            )
            ...
            if isinstance(net_output, tuple):
                ...
                # Normalize D_std to mean=1 (spatial pattern only)
                D_std_mean = D_std.mean()
                if D_std_mean > 1e-8:
                    D_std_norm = D_std / D_std_mean
                else:
                    D_std_norm = torch.ones_like(D_std)
                spatial_weight = spatial_alpha * D_std_norm + (1 - spatial_alpha)
                x_hat = x_cur + noise_scale * spatial_weight * noise
            else:
                x_hat = x_cur + noise_scale * noise
        else:
            x_hat = x_cur + noise_scale * noise

        # Euler step — denoising (no variance needed here)
        x_hat_batch = _apply_wrapper_select(input=x_hat, patching=patching)(
            patching=patching, input=x_hat
        ).to(latents.device)
        x_lr = x_lr.to(latents.device)
